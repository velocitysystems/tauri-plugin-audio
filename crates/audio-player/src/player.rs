...
impl RodioAudioPlayer {
   ...
   ///
   /// Opens the default audio output device on a dedicated thread. Takes one
   /// callback per event channel:
   ///
   ...
   fn advance_to(
      ...
      let (was_playing, was_paused, item_src) = {
         let mut inner = lock_inner(&self.inner);
